...
from fold.modules import TransformerLayer
...
def prepare_reg():
    a = torch.load('data/megatron-contact-regression.pt')
    p  = torch.tensor(a['model']['contact_head.regression.weight'][-6:])
    print(p)
    p.squeeze_(0)
    print(p)
    b = {'model': {'contact_head.regression.weight': p.float(), 'contact_head.regression.bias': torch.tensor([-5.0391], device='cuda:0')}}

    print(b)
    torch.save(b,'data/megatron-contact-regression.pt')
